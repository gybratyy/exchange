...
import { UploadCloud, XCircle, ChevronDown} from 'lucide-react'; // Added ChevronDown, CheckSquare, Square
...
export const BlogForm = ({ closeModal }) => {
    ...
    useEffect(() => {
        // Fetch categories when the component mounts or if they are not already loaded
    ...
    const handleSubmit = async (e) => {
        ...
        const newBlogData = {
            ...
            categories: selectedCategoryNames, // Send array of selected category names
        };
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-1">
            ...
            <div>
                ...
                <div className="mt-1 flex flex-col items-center">
                    ...
                    >
                        {imagePreview ? (
                            <>
                                <img src={imagePreview} alt="Предпросмотр" className="max-h-full max-w-full object-contain rounded-md" />
                                <button
                                    type="button"
                                    onClick={(e) => {
                                        e.stopPropagation();
            ...
            <div>
                ...
                <div className="relative">
                    ...
                    >
                        <span className={selectedCategoryNames.length > 0 ? "text-gray-900" : "text-gray-400"}>
                            {selectedCategoryNames.length > 0 ? selectedCategoryNames.join(', ') : 'Изберете категории'}
                        </span>
                    ...
                    {showCategoryDropdown && (
                        <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                            {availableCategoriesFromStore && availableCategoriesFromStore.length > 0 ? (
                                ...
                                <div className="px-3 py-2 text-sm text-gray-500">Could not find categories</div>
                            )}
                        </div>
                    )}
                </div>
...
import {useBlogStore} from '../store/useBlogStore';
import {useBookStore} from '../store/useBookStore'; // Assuming categories come from book store
import toast from 'react-hot-toast';
import {ChevronDown, UploadCloud, XCircle} from 'lucide-react';

export const BlogForm = ({closeModal}) => {
    const {createBlog} = useBlogStore();
    // Categories are fetched from useBookStore as per your current code
    const {categories: availableCategoriesFromStore, getCategories} = useBookStore();
    ...
    // This state will now store the full category objects that are selected
    const [selectedCategories, setSelectedCategories] = useState([]);
    ...
    // Modified to handle category objects
    ...
    const handleSubmit = async (e) => {
        ...
        // Extract only the _ids for submission
        ...
        const newBlogData = {
            ...
            text,
            image: imageBase64 || undefined,
            categories: categoryIdsToSend, // Send array of selected category _ids
        };

        try {
            const result = await createBlog(newBlogData);
            // Check if result exists and if it indicates success (or no error)
            // Also, check if result.data exists, as some successful creations might return the created object
            if (result && (result.success || !result.error || result.data)) {
                toast.success('Блог успешно создан!');
                closeModal();
            } else {
    ...
    // For displaying names in the button
