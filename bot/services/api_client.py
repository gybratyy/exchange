...
def _make_request(method: str, endpoint: str, cookies: Optional[Dict] = None, json_data: Optional[Dict] = None,
...
def register_user(full_name: str, email: str, password: str) -> Dict[str, Any]:
    try:
        s = requests.Session()
        response = s.post(f"{BACKEND_URL}/api/auth/signup",
...
def update_user_preferences(cookies: Dict, category_ids: List[str]) -> Dict[str, Any]:
    return _make_request("POST", "/api/auth/update-preferences", cookies=cookies,
                         json_data={"preferences": category_ids})
        ...
        async with aiohttp.ClientSession(cookies=cookies) as session:
            async with session.get(url) as response:
                if response.status != 200:
                    ...
                    return {"success": False, "error": f"Error {response.status}: {text}",
