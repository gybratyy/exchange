...
def main():
    ...
    my_books_conv = ConversationHandler(
        ...
        states={
            ...
            MY_BOOKS_CHOOSE_BOOK_INDEX: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$"),
                                                        choose_book_index_handler)],
            MY_BOOKS_CONFIRM_DELETE: [
                MessageHandler(filters.Regex("^(да|нет|yes|no|Да|Нет)$"), confirm_delete_handler)],
            MY_BOOKS_EDIT_TITLE: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                  CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_DESCRIPTION: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                        CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_AUTHOR: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                   CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_DATE: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                 CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_LANGUAGE: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                     CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_CATEGORIES: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                       CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_IMAGE: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                  CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_TYPE: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                 CommandHandler("skip", skip_edit_field_handler)],
            MY_BOOKS_EDIT_PRICE: [MessageHandler(
                filters.TEXT & ~filters.COMMAND & ~filters.Regex("^❌ Отмена$") & ~filters.Regex(
                    CANCEL_BOOK_CREATION_REGEX), universal_edit_field_handler),
                                  CommandHandler("skip", skip_edit_field_handler)],
        },
    ...
    profile_conv = ConversationHandler(
        ...
        states={
            PROFILE_WAITING_FOR_PIC: [
                ...
                MessageHandler(filters.Regex("^❌ Отмена$") | (filters.COMMAND & filters.Regex("^/cancel$")),
                               profile_handlers.cancel_profile_update_action),
            ],
        },
    ...
    recommendations_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(f"^💡 (Рекомендации книг|Мои рекомендации)$"),
